...
void io_display(dungeon_t *d)
{
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
	...
	attroff(COLOR_PAIR(d->charmap[y][x]->get_color()));
      ...
      }else if (d->objectMap[y][x] && d->objectMap[y][x]->visible()) {
	//	printf("found object \n");
	//	getch(); 
        attron(COLOR_PAIR(d->objectMap[y][x]->get_color()));
        mvaddch(y + 1, x, d->objectMap[y][x]->get_symbol());
        attroff(COLOR_PAIR(d->objectMap[y][x]->get_color()));
      }
      else {
